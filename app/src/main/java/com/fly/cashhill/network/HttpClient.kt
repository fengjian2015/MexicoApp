...
class HttpClient private constructor() {
    ...
    public fun initOkHttpClient(isAuth :Boolean):OkHttpClient {
        ...
        builder1
            ...
            .addInterceptor(com.fly.cashhill.network.SignInterceptor())
        if (isAuth){
            builder1.addInterceptor(GzipRequestInterceptor())
        }
        if (BuildConfig.DEBUG) {
            val loggingInterceptor = com.fly.cashhill.network.HttpLoggingInterceptor("HttpsLog")
            loggingInterceptor.setPrintLevel(com.fly.cashhill.network.HttpLoggingInterceptor.Level.BODY)
